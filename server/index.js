...
async function loadQuickCommands() {
  const profile = await detectCommandProfile();
  const fileName = `${profile}.json`;

  try {
    return { profile, fileName, commands: await readCommandFile(fileName) };
  } catch {
    return { profile: 'Other', fileName: 'Other.json', commands: await readCommandFile('Other.json') };
  }
}

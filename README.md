...
- Smooth page transitions and responsive toolbar wrapping so controls stay inside the frame.
...
Set `NO_OPEN=1` if you do not want the starter to open your browser automatically. Runtime details are visible at `/settings` and `/debug` after login. Set `NOVA_STARTUP_COMMAND=""` to disable the visible first command; by default Nova runs `cd ~` after the terminal appears so the user can see the shell move home.
